        ...
        jQuery('form#contactForm').submit(function () {
            ...
            jQuery('.requiredField').each(function () {
                if (jQuery.trim(jQuery(this).val()) == '') {
                    ...
                    jQuery(this).parent().append('<span class="error"><?php _e( '
                    You
                    forgot
                    to
                    enter
            ...
            if (!hasError) {
                ...
                jQuery.post(jQuery(this).attr('action'), formInput, function (data) {
                    jQuery('form#contactForm').slideUp("fast", function () {
                        jQuery(this).before('<p class="tick"><?php _e( ' < strong > Thanks
                        ! < /strong> Your email was successfully sent.', 'woothemes' ); ?></
                        p > ' );
                    });
                });
            }
            ...
            <section class="entry">
                ...
                <div class="location-twitter fix">
                    ...
                    <section id="office-location">
                        ...
                        <!--</ul>-->
                        Email <a href="mailto:info@orchidmate.com?Subject=Hello%20OrchidMate" target="_top">info@orchidmate.com</a>
                        or call us at (408) 748-4000 today!
                    </section>
                    <?php } ?>
                ...
                <ol class="forms">
                    <li><label for="contactName"><?php _e( 'Name', 'woothemes' ); ?></label>
                        <input type="text" name="contactName" id="contactName"
                               value="<?php if( isset( $_POST['contactName'] ) ) { echo esc_attr( $_POST['contactName'] ); } ?>"
                    ...
                    <div class="row">
                        <li><label for="phone"><?php _e( 'Phone', 'woothemes' ); ?></label>
                            <input type="text" name="phone" id="phone"
                                   value="<?php if( isset( $_POST['phone'] ) ) { echo esc_attr( $_POST['phone'] ); } ?>"
                                   class="txt"/>
                        </li>

                        <li><label for="ordernumber"><?php _e( 'Order Number', 'woothemes' ); ?></label>
                            <input type="text" name="ordernumber" id="ordernumber"
                                   value="<?php if( isset( $_POST['ordernumber'] ) ) { echo esc_attr( $_POST['ordernumber'] ); } ?>"
                                   class="txt requiredField email"/>
                        </li>
                    </div>
                    ...
                    <li class="textarea"><label for="commentsText"><?php _e( 'Message', 'woothemes' ); ?></label>
                        <textarea name="comments" id="commentsText" rows="20" cols="30"
                                  class="requiredField"><?php if( isset( $_POST['comments'] ) ) { echo esc_textarea( $_POST['comments'] ); } ?></textarea>
                        <?php if( $commentError != '' ) { ?>
                        <span class="error"><?php echo $commentError; ?></span>
                        <?php } ?>
                    </li>
                    <li class="inline"><input type="checkbox" name="sendCopy" id="sendCopy"
                                              value="true"<?php if( isset( $_POST['sendCopy'] ) && $_POST['sendCopy'] == true ) { echo ' checked="checked"'; } ?>
                        /><label
                                for="sendCopy"><?php _e( 'Send a copy of this email to yourself', 'woothemes' ); ?></label>
                    </li>
                    <li class="screenReader"><label for="checking"
                                                    class="screenReader"><?php _e( 'If you want to submit this form, do not enter anything in this field', 'woothemes' ); ?></label><input
                            type="text" name="checking" id="checking" class="screenReader"
                            value="<?php if( isset( $_POST['checking'] ) ) { echo esc_attr( $_POST['checking'] ); } ?>"/>
                    </li>
                    <li class="buttons"><input type="hidden" name="submitted" id="submitted" value="true"/><input
                            class="submit button" type="submit" value="<?php esc_attr_e( 'Submit', 'woothemes' ); ?>"/>
                    </li>
                </ol>
            </form>
