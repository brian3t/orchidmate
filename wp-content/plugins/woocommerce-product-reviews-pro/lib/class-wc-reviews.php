...
class WC_Reviews {
	...
	public function __construct() {
		...
		require_once( 'wc-review-functions.php' );
	...
	public function add_menu_items() {
		...
		$page = add_submenu_page(
			'woocommerce',
			__( 'Reviews', 'wc-product-reviews-pro' ),
			$label,
			'edit_posts',
			'reviews',
	...
	public function add_reviews_screen_options() {
		...
		add_screen_option( 'per_page', array( 'label' => _x( 'Reviews', 'reviews per page (screen options)', 'wc-product-reviews-pro' ) ) );
	}
	...
	public function render_reviews_list_table() {
		...
		<div class="wrap">
			...
			<form id="reviews-filter" method="get">
				...
				<?php $this->reviews_list_table->search_box( __( 'Search reviews', 'wc-product-reviews-pro' ), 'reviews' ); ?>
	...
	public function display_messages() {
		...
		if ( isset( $_REQUEST['approved'] ) || isset( $_REQUEST['deleted'] ) || isset( $_REQUEST['trashed'] ) || isset( $_REQUEST['untrashed'] ) || isset( $_REQUEST['spammed'] ) || isset( $_REQUEST['unspammed'] ) || isset( $_REQUEST['same'] ) ) {
			...
			if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
				if ( $approved > 0 ) {
					$messages[] = sprintf( _n( '%s comment approved', '%s comments approved', $approved, 'wc-product-reviews-pro' ), $approved );
				}

				if ( $spammed > 0 ) {
					$ids = isset($_REQUEST['ids']) ? $_REQUEST['ids'] : 0;
					$messages[] = sprintf( _n( '%s comment marked as spam.', '%s comments marked as spam.', $spammed, 'wc-product-reviews-pro' ), $spammed ) . ' <a href="' . esc_url( wp_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", "bulk-comments" ) ) . '">' . __('Undo', 'wc-product-reviews-pro') . '</a><br />';
				}

				if ( $unspammed > 0 ) {
					$messages[] = sprintf( _n( '%s comment restored from the spam', '%s comments restored from the spam', $unspammed, 'wc-product-reviews-pro' ), $unspammed );
				}

				if ( $trashed > 0 ) {
					$ids = isset($_REQUEST['ids']) ? $_REQUEST['ids'] : 0;
					$messages[] = sprintf( _n( '%s comment moved to the Trash.', '%s comments moved to the Trash.', $trashed, 'wc-product-reviews-pro' ), $trashed ) . ' <a href="' . esc_url( wp_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", "bulk-comments" ) ) . '">' . __('Undo', 'wc-product-reviews-pro') . '</a><br />';
				}

				if ( $untrashed > 0 ) {
					$messages[] = sprintf( _n( '%s comment restored from the Trash', '%s comments restored from the Trash', $untrashed, 'wc-product-reviews-pro' ), $untrashed );
				}

				if ( $deleted > 0 ) {
					$messages[] = sprintf( _n( '%s comment permanently deleted', '%s comments permanently deleted', $deleted, 'wc-product-reviews-pro' ), $deleted );
				}

				if ( $same > 0 && $comment = get_comment( $same ) ) {
					switch ( $comment->comment_approved ) {
						case '1' :
							$messages[] = __( 'This comment is already approved.', 'wc-product-reviews-pro' ) . ' <a href="' . esc_url( admin_url( "comment.php?action=editcomment&c=$same" ) ) . '">' . __( 'Edit comment', 'wc-product-reviews-pro' ) . '</a>';
							break;
						case 'trash' :
							$messages[] = __( 'This comment is already in the Trash.', 'wc-product-reviews-pro' ) . ' <a href="' . esc_url( admin_url( 'edit-comments.php?comment_status=trash' ) ) . '"> ' . __( 'View Trash', 'wc-product-reviews-pro' ) . '</a>';
							break;
						case 'spam' :
							$messages[] = __( 'This comment is already marked as spam.', 'wc-product-reviews-pro' ) . ' <a href="' . esc_url( admin_url( "comment.php?action=editcomment&c=$same" ) ) . '">' . __( 'Edit comment', 'wc-product-reviews-pro' ) . '</a>';
							break;
					}
				}
	 ...
	 *
	 * @param  array List of screen IDs
	 * @return array
	 */
	public function add_review_screen_id( $screen_ids ) {

		$wc_screen_id = sanitize_title( __( 'WooCommerce', 'wc-product-reviews-pro' ) );
	...
	public function filter_edit_comments_screen_translations( $translation, $text ) {
		...
		if ( $comment && 'product' == get_post_type( $comment->comment_post_ID ) ) {
			...
			switch ( $text ) {
				case 'Edit Comment':
					$translation = __( 'Edit Review', 'wc-product-reviews-pro' );
					break;
				case 'Moderate Comment':
					$translation = __( 'Moderate Review', 'wc-product-reviews-pro' );
					break;
			}
		}
