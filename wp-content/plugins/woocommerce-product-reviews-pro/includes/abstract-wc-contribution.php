...
abstract class WC_Contribution {
	...
	/** @public int Contribution (comment) ID */
	public $id;

	/** @public int related Product (post) ID */
	public $product_id;

	/** @public string contributor name */
	public $contributor_name;

	/** @public string contributor email */
	public $contributor_email;

	/** @public string contributor IP */
	public $contributor_ip;

	/** @public string contribution date */
	public $contribution_date;

	/** @public string contribution date in gmt */
	public $contribution_date_gmt;

	/** @public string contribution content (text) */
	public $content;

	/** @public int net contribution vote, used as a roll-up of the sum of positive_votes/negative_votes meta in order to improve sort performance */
	public $karma;

	/** @public int contribution moderation status (0 = not approved, 1 = approved, 2 = flagged as inappropriate) */
	public $moderation;

	/** @public string contribution type */
	public $type;

	/** @public int contribution parent (used for threaded comments) */
	public $parent;

	/** @public int contributor (user) ID (if contributor was logged in) */
	public $contributor_id;

	/** @public string contribution_title (not used for questions or comments) */
	public $title;

	/** @public float numeric rating (only used for reviews) */
	public $rating;

	/** @public int number of positive votes */
	public $positive_votes;

	/** @public int number of negative votes */
	public $negative_votes;

	/** @public int number of times this contribution flagged inappropriate */
	public $flag_count;

	/** @public int attached media ID (not used for comments) */
	public $attachment_id;

	/** @public int attached media URL (not used for comments) */
	public $attachment_url;

	/** @public int attached media type (not used for comments) */
	public $attachment_type;

	/** @public object The actual comment object **/
	public $comment;

	/** @public string Failure message for vote and flag methods **/
	...
	public function cast_vote( $type = 'positive', $user_id = '' ) {
		...
		if ( ! $user_id ) {
			$this->_failure_message = __( 'You must be logged in to vote', WC_Product_Reviews_Pro::TEXT_DOMAIN );
			return false;
		}

		// Users are not allowed to vote for their own contribution
		if ( $user_id == $this->contributor_id ) {
			$this->_failure_message = __( 'You can\'t vote for yourself ;(', WC_Product_Reviews_Pro::TEXT_DOMAIN );
			return false;
		}
	...
	public function flag( $reason = null, $user_id = '' ) {
		...
		if ( $this->has_user_flagged( $user_id ) ) {
			$this->_failure_message = __( 'You have already flagged this contribution', WC_Product_Reviews_Pro::TEXT_DOMAIN );
			return false;
		}

		// Users are not allowed to flag for their own contribution
		if ( is_user_logged_in() && $user_id == $this->contributor_id ) {
			$this->_failure_message = __( 'You can\'t flag your own contributions!', WC_Product_Reviews_Pro::TEXT_DOMAIN );
			return false;
		}
